...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            name="load_model",
            description=(
                "Restore a saved sktime estimator from a local path or MLflow URI "
                "and register it into the handle registry for downstream tools."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": (
                            "Directory path or MLflow URI to load the model from. "
                            "Example: '/tmp/my_model' or 'mlflow-artifacts:/run_id/model'"
                        ),
                    },
                },
                "required": ["path"],
